<div class="grid gap-5 md:grid-cols-2">
    <div class="md:col-span-2">
        ...
        >
            ...
            @foreach ($mataKuliahs as $mataKuliah)
                <option
                    value="{{ $mataKuliah->id }}"
                    @selected(old('matkul_id', $jadwalKuliah?->matkul_id) == $mataKuliah->id)
                >
    ...
    <div>
        ...
        >
            ...
            @foreach ($ruangans as $ruangan)
                <option
                    value="{{ $ruangan->id }}"
                    @selected(old('ruangan_id', $jadwalKuliah?->ruangan_id) == $ruangan->id)
                >
            ...
            @foreach ($tahunAkademiks as $tahunAkademik)
                <option
                    ...
                    @selected(
                        old(
                            'tahun_akademik_id',
                            $jadwalKuliah?->tahun_akademik_id
                        ) == $tahunAkademik->id
                    )
                >
                    {{ $tahunAkademik->tahun }}
    ...
    <div>
        ...
        >
            ...
            @foreach (['Senin', 'Selasa', 'Rabu', 'Kamis', 'Jumat', 'Sabtu'] as $hari)
                <option
                    value="{{ $hari }}"
                    @selected(old('hari', $jadwalKuliah?->hari) === $hari)
                >
                    {{ $hari }}
                </option>
            @endforeach
    ...
    <div>
        ...
        <input
            ...
            id="jam_mulai"
            name="jam_mulai"
            value="{{ old('jam_mulai', $jadwalKuliah ? substr($jadwalKuliah->jam_mulai, 0, 5) : '') }}"
            required
            class="w-full rounded-md border border-gray-300 px-3 py-2 text-sm outline-none focus:border-blue-600 focus:ring-1 focus:ring-blue-600"
        >
    </div>
    ...
    <div>
        ...
        <input
            ...
            id="jam_selesai"
            name="jam_selesai"
            value="{{ old('jam_selesai', $jadwalKuliah ? substr($jadwalKuliah->jam_selesai, 0, 5) : '') }}"
            required
            class="w-full rounded-md border border-gray-300 px-3 py-2 text-sm outline-none focus:border-blue-600 focus:ring-1 focus:ring-blue-600"
        >
    </div>
